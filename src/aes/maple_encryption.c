#include <string.h>
#include <stdio.h>
#include "maple_encryption.h"
#include "aes.h"

const int GAME_VERSION = 79;

const unsigned char AES_KEYS[32] =
{
	0x13, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0xB4, 0x00, 0x00, 0x00,
	0x1B, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00
};

const unsigned char IV_TABLE[256] =
{
...
{
    shifts &= 7;
    return (unsigned char)((val >> shifts) | (val << (8 - shifts)));
}
...
void shuffle_iv(unsigned char *iv)
{
    ...
    for (; loop_counter < 4; loop_counter++)
    {
        ...
        new_iv[0] = (unsigned char)(shift & 0xFFu);
        new_iv[1] = (unsigned char)((shift >> 8) & 0xFFu);
        new_iv[2] = (unsigned char)((shift >> 16) & 0xFFu);
        new_iv[3] = (unsigned char)((shift >> 24) & 0xFFu);
    }
...
void aes_crypt(unsigned char *buffer, unsigned char *iv, unsigned short size)
{
    ...
    while (size > pos)
    {
        ...
        aes_encrypt_key256(AES_KEYS, cx);
...
{
    unsigned short version = (((iv[3] << 8) | iv[2]) ^ -(GAME_VERSION + 1));
    size = version ^ size;
